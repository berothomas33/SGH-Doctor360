...
open class ProcdeureDropDown : UITextField{
    
    var arrow : ProcedeureLab!
    var table : UITableView!
    var shadow : UIView!
    public  var selectedIndex: Int?
    
    
    var ListOFDrugsloa : [labTestDoctorlist]?
    ...
    var searchText = String() {
        didSet{
            if searchText == "" {
              ...
              //  FilterWithApi(SerchText: searchText)
                if(searchText.count > 1 && searchText.count < 4) {
                                   FilterWithApi(SerchText: searchText) } else if (searchText.count > 4) {
    ...
    public func FilterWithApi(SerchText:String)  {
        ...
        API.Procdeurelist( Search : searchText ,completion:  { (respons, error) in
            ...
            if(error == "" ) {
                ...
                self.ListOFDrugsloa = respons as! [labTestDoctorlist]
                for item in self.ListOFDrugsloa! {
                    
                    var dictx   = self.convertToDictionary(text:item.toJSONString()!)

                    print(dictx)


                    var Duar = ProcdeureSaveed(ProcdeuureID: item.id!, Qty: 1, Remarks: "",Name: item.name!)
                    ...
                    self.optionArray.append(item.name!)
                    
                    self.optionIds?.append(item.id!)
...
//        arrow = ProcedeureLab(origin: CGPoint(x: center.x - arrowSize/2,y: center.y - arrowSize/2),size: arrowSize  )
...
//        self.backgroundView.backgroundColor = .clear
        ...
        let size = self.frame.height
               ...
               self.rightViewMode = .always
               let arrowContainerView = UIView(frame: rightView.frame)
                let arrowContainerView2 = UIView(frame: rightView.frame)
               let center2 = arrowContainerView2.center

               
               let imageName = "search"
               let image = UIImage(named: imageName)
               let imageViewx = UIImageView(image: image!)
               
                imageViewx.frame = CGRect(x: ( center2.x - center2.x / 2) , y: center2.y - (center2.y / 2) , width: 30, height: 30)
               ...
               arrowContainerView2.addSubview(imageViewx)
               
               self.rightView?.addSubview(arrowContainerView2)
    ...
    public func showList() {
        if parentController == nil{
            parentController = self.parentViewControllerab
        }
        ...
        UIView.animate(withDuration: 0.9,
                       ...
                       animations: { () -> Void in
                        ...
                        self.shadow.dropShadowlab()
                        self.arrow.PostProcedeure = .up
    ...
    public func hideList() {
        ...
        UIView.animate(withDuration: 1.0,
                       ...
                       animations: { () -> Void in
                        ...
                        self.shadow.alpha = 0
                        self.shadow.frame = self.table.frame
                        self.arrow.PostProcedeure = .down
        },
...
extension ProcdeureDropDown : UITextFieldDelegate {
...
extension ProcdeureDropDown: UITableViewDataSource {
...
extension ProcdeureDropDown: UITableViewDelegate {
...
class ProcedeureLab: UIView {
    ...
    var PostProcedeure: PostProcedeure = .down {
        didSet{
            switch PostProcedeure {
            case .left:
...
extension UIView {
    ...
    func dropShadowlab(scale: Bool = true) {
    ...
    var parentViewControllerab: UIViewController? {
