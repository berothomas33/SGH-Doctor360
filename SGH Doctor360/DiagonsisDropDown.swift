...
open class DiagonsisDropDown : UITextField{
    
    var arrow : ArrowDiago!
    var table : UITableView!
    var shadow : UIView!
    public  var selectedIndex: Int?
    
    
    var ListOFDrugsloa = [DiagonDoctorlist]()
    ...
    func FilterWithApi(SerchText:String) {
        ...
        API.DiagonsisListsapi( Search : searchText ,completion:  { (respons, error) in
            ...
            if(error == "" ) {
                ...
                self.ListOFDrugsloa = respons as! [DiagonDoctorlist]
                for item in self.ListOFDrugsloa {
                    ...
                    var Duar = DiagonsicSaved(ID: item.id!, CODE: item.code!, Descriptio: item.description!)
                    RealmService.shared.create(Duar)
                    
                    
                    
                    self.optionArray.append(item.description!)
...
//        imageViewx.frame = CGRect(x:  center2.x , y: center2.y - (center2.y / 2) , width: 30, height: 30)
        ...
        arrow = ArrowDiago(origin: CGPoint(x: center.x - arrowSize/2,y: center.y - arrowSize/2),size: arrowSize  )
    ...
    public func showList() {
        if parentController == nil{
            parentController = self.parentViewControllerDiago
        }
        ...
        UIView.animate(withDuration: 0.9,
                       ...
                       animations: { () -> Void in
                        ...
                        self.shadow.dropShadowDiago()
                        self.arrow.PositionDiago = .up
...
extension DiagonsisDropDown : UITextFieldDelegate {
...
extension DiagonsisDropDown: UITableViewDataSource {
...
extension DiagonsisDropDown: UITableViewDelegate {
...
class ArrowDiago: UIView {
    ...
    var PositionDiago: PositionDiago = .down {
        didSet{
            switch PositionDiago {
            case .left:
...
extension UIView {
    ...
    func dropShadowDiago(scale: Bool = true) {
    ...
    var parentViewControllerDiago: UIViewController? {
